import { NextResponse } from 'next/server'
import { supabaseAdmin } from '@/lib/supabase-admin'
import { requireCliente } from '@/lib/cliente-api-auth'

export const runtime = 'nodejs'

function erro(message, status = 400) {
  return NextResponse.json({ ok: false, error: message }, { status })
}

function limparUuid(value = '') {
  const text = String(value || '').trim()
  return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(text)
    ? text
    : ''
}

async function getParamId(params) {
  const resolved = typeof params?.then === 'function' ? await params : params
  return Array.isArray(resolved?.id) ? resolved.id[0] : resolved?.id
}

function aplicarEscopoContrato(query, { clienteId, empresaId }) {
  if (empresaId && clienteId) {
    return query.or(`empresa_id.eq.${empresaId},cliente_id.eq.${clienteId}`)
  }

  if (empresaId) return query.eq('empresa_id', empresaId)
  return query.eq('cliente_id', clienteId)
}

async function carregarContrato({ contratoId, auth, includeHtml = true }) {
  const { cliente, empresaId } = auth
  const clienteId = cliente.id

  let query = supabaseAdmin
    .from('empresa_contratos')
    .select(includeHtml ? '*' : 'id, empresa_id, cliente_id, status, titulo, cliente_email, sent_to_cliente_at, accepted_at, accepted_by_email, created_at, updated_at')
    .eq('id', contratoId)

  query = aplicarEscopoContrato(query, { clienteId, empresaId })

  const { data, error } = await query.maybeSingle()

  if (error) throw error
  return data
}

async function registrarEvento({ contratoId, eventType, auth, metadata = {} }) {
  try {
    await supabaseAdmin
      .from('empresa_contrato_events')
      .insert({
        contrato_id: contratoId,
        event_type: eventType,
        actor_id: auth.user?.id || null,
        actor_email: auth.user?.email || auth.cliente?.email || null,
        metadata,
      })
  } catch (error) {
    console.warn('Evento de contrato não registrado:', error.message)
  }
}

export async function GET(request, context) {
  const auth = await requireCliente(request)
  if (auth.errorResponse) return auth.errorResponse

  try {
    const id = await getParamId(context?.params)
    const contratoId = limparUuid(id)

    if (!contratoId) return erro('ID do contrato inválido.')

    const contrato = await carregarContrato({ contratoId, auth })

    if (!contrato) {
      return erro('Contrato não encontrado para este cliente.', 404)
    }

    let eventos = []

    try {
      const { data } = await supabaseAdmin
        .from('empresa_contrato_events')
        .select('id, event_type, actor_email, metadata, created_at')
        .eq('contrato_id', contrato.id)
        .order('created_at', { ascending: false })
        .limit(30)

      eventos = data || []
    } catch {
      eventos = []
    }

    return NextResponse.json({
      ok: true,
      contrato,
      eventos,
    })
  } catch (error) {
    return NextResponse.json(
      { ok: false, error: error.message || 'Erro ao carregar contrato.' },
      { status: 500 }
    )
  }
}

export async function POST(request, context) {
  const auth = await requireCliente(request)
  if (auth.errorResponse) return auth.errorResponse

  try {
    const id = await getParamId(context?.params)
    const contratoId = limparUuid(id)

    if (!contratoId) return erro('ID do contrato inválido.')

    const contrato = await carregarContrato({ contratoId, auth })

    if (!contrato) {
      return erro('Contrato não encontrado para este cliente.', 404)
    }

    if (contrato.status === 'cancelado') {
      return erro('Contrato cancelado não pode ser aceito.', 422)
    }

    const now = new Date().toISOString()
    const acceptedEmail = auth.user?.email || auth.cliente?.email || contrato.cliente_email || null

    const { data: atualizado, error } = await supabaseAdmin
      .from('empresa_contratos')
      .update({
        status: 'assinado',
        accepted_at: now,
        accepted_by_email: acceptedEmail,
        updated_by: auth.user?.id || null,
      })
      .eq('id', contrato.id)
      .select('*')
      .maybeSingle()

    if (error) throw error

    await registrarEvento({
      contratoId: contrato.id,
      eventType: 'contrato_aceito_cliente',
      auth,
      metadata: { accepted_at: now, accepted_by_email: acceptedEmail },
    })

    return NextResponse.json({
      ok: true,
      contrato: atualizado,
      message: 'Contrato aceito com sucesso.',
    })
  } catch (error) {
    return NextResponse.json(
      { ok: false, error: error.message || 'Erro ao aceitar contrato.' },
      { status: 500 }
    )
  }
}
